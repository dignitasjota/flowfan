...
- [ ] **WK-8 · Carrera de rotación del refresh token de Twitter** — _DIFERIDO: el lock correcto exige re-leer el account de DB dentro del refresh y persistir bajo lock, tocando los 4 call-sites y el patrón actual (la función recibe strings cifrados, no toca DB). Un lock best-effort sin re-lectura no elimina la corrupción del token rotativo (da falsa seguridad). Se aborda junto con ARCH-2 (pollers→repeatable jobs, punto único de refresh)._ `src/server/services/twitter-publisher.ts:13-57`, `twitter-poller.ts:100-116`, `worker.ts:1251-1268` — Twitter rota el refresh token en cada uso; si coinciden un tick del poller y un scheduled post, ambos usan el mismo refresh token → Twitter invalida la familia → **cuenta desconectada hasta re-OAuth**. **Fix:** lock distribuido en Redis por `accountId` (`SET NX EX 30`) alrededor del refresh, o refresh en un único punto.
