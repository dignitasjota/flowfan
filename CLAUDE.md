...
## Infraestructura transversal (helpers compartidos)

Utilidades reutilizables introducidas/consolidadas durante la remediación de la auditoría (ver [`AUDITORIA_BACKLOG.md`](./AUDITORIA_BACKLOG.md)). Preferir estas antes de reinventar el patrón.

- **`src/lib/redis-connection.ts` → `getRedisConnectionOptions()`** — factoría única de conexión Redis para BullMQ que parsea la **URL completa** de `REDIS_URL` (host, puerto, `username`, `password`, `db`, `tls` para `rediss://`) con `maxRetriesPerRequest: null`. La usan las 12 colas (`src/server/queues/index.ts`) y todos los workers (`worker.ts`). No construir `{ host, port }` a mano — descarta credenciales/TLS y rompe en producción con auth (WK-9 / ARCH-10).
- **`src/lib/ssrf.ts` → `assertPublicHttpUrl(url)` / `isPrivateIp(ip)`** — anti-SSRF: resuelve el host y rechaza IPs privadas/loopback/link-local antes de hacer `fetch` a URLs controladas por el usuario. Usar `redirect: "manual"` junto a esta comprobación. Ya aplicado en `webhook-dispatcher` (SEC-2) y `blog-to-social.extractContent` (TEN-10).
- **`src/lib/request-ip.ts` → `getClientIp(req)`** — extrae la IP real del cliente respetando `TRUSTED_PROXY_HOPS` (número de proxies de confianza) en vez de confiar ciegamente en el primer valor de `X-Forwarded-For`. Usar en handlers que hacen rate limiting por IP (register/forgot/reset). Nginx debe enviar `proxy_set_header X-Forwarded-For $remote_addr` (ver `DEPLOYMENT.md`).
- **`src/lib/rate-limit.ts`** — sliding window en Redis. La config acepta `failClosed?: boolean`: cuando es `true` (presets `auth`/`register`), un fallo de Redis deniega en vez de dejar pasar (fail-closed en superficies sensibles). El middleware de API keys (`api-key-auth.ts`) usa `rateLimit(\`apikey:${keyId}\`, ...)` en Redis, no un `Map` en memoria (SEC-3).
